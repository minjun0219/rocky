...
`ROCKY_*` 변수는 **전체 표면 서버 진입점 (`src/index.ts`) 전용** — 이를 실행하는 모든 호스트(Claude Code plugin / Codex / opencode)에 적용되고, standalone `openapi-mcp` CLI 는 인지하지 않는다 (CLI 는 `openapi-mcp.json` config 파일 + XDG 표준 변수만 본다).
...
| `ROCKY_WORKLOG_AUTO_CAPTURE` | `1` (on) | 전체 표면 서버 | `Stop` hook 의 턴 자동 기록 on/off. `0` / `false` / `off` / `no` 값만 비활성, 그 외는 활성. `rocky.json` 의 `worklog.autoCapture` 보다 우선. |
