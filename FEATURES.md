...
MCP tool · 슬래시 커맨드와 별개로, Claude Code plugin 은 `hooks/hooks.json` 에 `Stop` hook 하나를 등록한다 (v0.9). 매 턴(응답) 종료 시 트랜스크립트에서 이번 턴을 추출해 워크로그에 `kind:"turn"` 항목을 자동으로 남긴다 — 결정론적(LLM 미사용)이며, 어떤 실패도 턴을 막지 않도록 항상 exit 0 으로 끝난다. 구현은 `src/hooks/log-turn.ts` (트랜스크립트 파싱은 `src/hooks/transcript.ts` 의 `extractTurn` / `buildTurnContent`).
...
미지원 top-level 키는 즉시 reject 된다 (`$schema` / `openapi` / `seo` / `worklog` 만 허용 — `rocky.schema.json` 최상위 `additionalProperties:false` 와 런타임 `validateConfig` 둘 다 강제) — 오타 가드. 새 도메인이 재추가될 때는 이 허용 목록과 스키마를 함께 갱신해야 한다.
