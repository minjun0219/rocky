...
1. 이 저장소를 마켓플레이스로 설치: `claude plugin marketplace add <repo>` → `claude plugin install rocky@rocky-marketplace` (저장소 자체가 소스, 파사드 없음). 코드 변경은 `/reload-plugins` 로 반영. 웹/원격 세션에서는 `claude plugin marketplace add minjun0219/rocky` 처럼 GitHub 소스로 추가.
