...
- **단일 패키지 (`@minjun0219/agent-toolkit`) — 두 배포 타깃, 동일한 7-tool surface**:
  | 배포 타깃 | 역할 | 설치 |
  | --- | --- | --- |
...
- **설정 파일**:
  - `agent-toolkit.json` — plugin 이 읽는다 (project 의 `./.opencode/agent-toolkit.json` 이 user 의 `~/.config/opencode/agent-toolkit/agent-toolkit.json` 을 leaf 단위로 덮어쓴다). v0.3 부터 `openapi.registry` 한 키만 존재.
...
> - v0.2 까지 존재하던 journal / mysql / notion / spec-pact / pr-watch 5 도메인 + rocky / grace / mindy 3 에이전트 + 5 스킬은 [`archive/pre-openapi-only-slim`](https://github.com/minjun0219/agent-toolkit/tree/archive/pre-openapi-only-slim) 브랜치에 박제되어 있다.
...
`AGENT_TOOLKIT_*` 변수는 **Claude Code plugin 진입점 (`src/index.ts`) 전용** — standalone `openapi-mcp` CLI 는 인지하지 않는다 (CLI 는 `openapi-mcp.json` config 파일 + XDG 표준 변수만 본다).

| 변수 | 기본값 | 적용 host | 영향 |
| --- | --- | --- | --- |
| `AGENT_TOOLKIT_OPENAPI_CACHE_DIR` | XDG cache (`$XDG_CACHE_HOME/openapi-mcp` 또는 `~/.cache/openapi-mcp`) | plugin | OpenAPI spec 디스크 캐시 위치. |
| `AGENT_TOOLKIT_OPENAPI_CACHE_TTL` | `300` (초) | plugin | spec 단위 `cacheTtlSeconds` 기본값으로 주입 — `agent-toolkit.json` 의 leaf 에 별도 TTL 이 없을 때 사용. |
| `AGENT_TOOLKIT_CONFIG` | `~/.config/opencode/agent-toolkit/agent-toolkit.json` | plugin | user-level `agent-toolkit.json` 경로 override. |
