...
- **단일 패키지 (`@minjun0219/rocky`) — 두 배포 타깃, 공유 7 openapi tool + plugin 전용 `seo_validate` + CLI-gated `notion_*` + 기록 `worklog_*` (`Stop` hook 자동 기록 + 정리 `/recall`)**:
  | 배포 타깃 | 역할 | 설치 |
  | --- | --- | --- |
