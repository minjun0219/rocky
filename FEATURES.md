...
1. 이 저장소를 마켓플레이스로 설치: `claude plugin marketplace add <repo>` → `claude plugin install rocky@rocky-marketplace` (저장소 자체가 소스, 파사드 없음). 코드 변경은 `/reload-plugins` 로 반영. 원격 세션에서는 `/plugin marketplace add minjun0219/rocky` 처럼 GitHub 소스로 추가하고, claude.ai 웹 UI 에서는 CLI 명령 대신 플러그인/마켓플레이스 설정에 저장소 URL (`https://github.com/minjun0219/rocky.git`) 을 등록.
