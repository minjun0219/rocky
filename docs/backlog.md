...
- **부활 방법**: 아래 완성본을 `skills/docs-lifecycle/SKILL.md` 로 옮기고 문서 동기화 4곳 — `FEATURES.md` 스킬 블록 / `README.md` 스킬 bullet / `AGENTS.md` Layout + *What rocky is* / `.claude-plugin/plugin.json` description — 을 갱신하면 된다 (스킬은 자동 발견, `.claude-plugin/plugin.json` 에 skills 필드 없음).
