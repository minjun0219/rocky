# Design — journal worklog 자동 기록 + `/history` 히스토리 다이제스트
...
2. **필요할 때 정리해 앵커로 쓴다** — 히스토리 파악이 필요한 시점에 `/history` 이
...
- **자동 read-back 없음** — 다음 세션에 워크로그/다이제스트를 자동 주입하지 않는다.
  히스토리는 `/history` / `journal_read` / `journal_search` 로 on-demand 조회만.
- **자동 정리(폴링/임계 자동 발화) 없음** — `/history` 은 여전히 호출식(반자동: 스킬이
...
[스킬이 히스토리 필요]  /history → 적응적 Haiku/Sonnet 서브에이전트
                                    → 앵커 다이제스트 생성 (id 참조 포함)
                                    → append(kind:"digest")  ← 다음 /history 의 watermark
                                    → 앵커 요약 보고
```
...
### 3. `commands/history.md` (신규, 기존 `curate.md` 대체)

`/history [주제 힌트]` — 워크로그를 앵커 히스토리 다이제스트로 정리하고 보고.

절차:
1. `journal_status` → path / totalEntries / lastEntryAt.
2. 마지막 watermark 조회: `journal_read { kind: "digest", limit: 1 }` → 있으면 그
   timestamp 가 watermark, 없으면 첫 실행.
...
6. `journal_append { kind: "digest", content: <다이제스트>, tags: ["digest"] }` — 이
   엔트리 자체가 다음 /history 의 watermark.
...
- `kind:"turn"` — Stop hook 자동 워크로그. `content = "req: … | tools: … | did: …"`,
  `tags:["turn"]`.
- `kind:"digest"` — `/history` 산출 앵커 다이제스트. 원본 엔트리 id 참조. timestamp 가
  다음 /history 의 증분 watermark 로 겸용.
- 기존 `decision`/`blocker`/`answer`/`note` 수동 엔트리와 공존 — 셋 다 `/history` 정리 대상.
...
- 저널은 append-only 불변 — hook 과 /history 모두 기존 줄을 수정/삭제하지 않는다.
...
- `commands/history.md` 는 산문 → 유닛테스트 없음, 수동 확인.

## 스코프 정합성 (AGENTS.md)

- 자동 **기록**(Stop hook) ≠ 자동 **정리**(`/history` 은 호출식) → "auto-curate(threshold/
  polling) Out" 조항과 무충돌.
...
5. `/reload-plugins` 로 로컬 검증 (hook 발화 + `/history` 왕복)
