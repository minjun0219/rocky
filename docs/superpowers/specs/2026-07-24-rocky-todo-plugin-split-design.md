...
- **`claude plugin install --from` 없음** ✗ (실측 `claude plugin install --help`) — `install <plugin>`
  ...
  rocky-todo 도 자기 marketplace 에 등록되어야 설치된다.
...
- **`dependencies` 는 자동 해석됨** ✓ (실증: `claude plugin prune` = "auto-installed dependencies
  제거" 커맨드 존재). rocky-todo 가 `dependencies:["rocky"]` 를 선언하면 rocky 자동 동반. 단
  **크로스-마켓플레이스 의존 자동 해석은 불확실** — rocky-todo 마켓과 rocky 마켓이 다르므로,
  스킬/문서가 "rocky 를 먼저 설치" 를 보강 안내한다.
...
- **marketplace 숨김 필드 없음** ✗ — 별도 레포·별도 마켓이라 애초에 rocky 브라우즈 목록에 안 뜬다.
  rocky-todo 는 자기 마켓을 `marketplace add` 한 사용자에게만 보인다 → 사실상의 분리 노출.
...
2. **rocky-todo 위치**: 신규 레포 `minjun0219/rocky-todo` (자기 marketplace + plugin.json + CI + release).
...
5. **의존성**: rocky-todo → `dependencies:["rocky"]` 명목 선언 + 스킬/문서가 "rocky 먼저" 안내
   (크로스-마켓 자동 해석 미보장).
...
├── .claude-plugin/marketplace.json   # rocky-todo-marketplace, plugin source "./"
...
- **rocky-todo (레포 B)**: AGENTS.md / FEATURES.md / README.md 신규(rocky 미러, todo 전용).
  `docs/rocky-todo.md` 재작성(설치=활성화, 자기 마켓 설치, `/mcp` retry 안내, CLI 표면).
...
- **추출 순서**: (1) 현재 브랜치 HEAD 에서 `git filter-repo` 로 todo 경로 history 추출 → 새 레포
  ...
  베이스)로 todo 삭제.
- **기존 rocky 설치 영향 없음** — rocky source 는 `"./"` 그대로, 구조 불변. 사용자는 rocky-todo 를
  쓰려면 새 마켓을 add 하면 된다. 메모리 [[rocky-local-plugin-facade]] 는 rocky 쪽 변화 없음(포인터만).
...
- **크로스-마켓 dependency 자동해석 미보장** — 스킬/문서 보강으로 커버.
