...
rocky 의 다른 도메인 없이 OpenAPI / Swagger spec MCP 만 띄우고 싶을 때 쓰는 단독 CLI 진입점. 구 [`openapi-mcp-server`](https://github.com/minjun0219/openapi-mcp-server) 와 동일한 config 형태와 7 tool 표면을 그대로 받는다.
...
```bash
git clone https://github.com/minjun0219/rocky.git
cd rocky
bun install
...
- Claude Code plugin: `input` = spec URL **또는** `host:env:spec` 핸들. `rocky.json` 의 `openapi.registry` 를 adapter 로 평탄화. baseUrl 은 leaf 에 옵션으로 선언.
