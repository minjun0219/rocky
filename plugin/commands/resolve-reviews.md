---
description: PR 에 이미 붙어 있는 리뷰(Copilot / Codex / 사람)를 해소한다 — 판단이 필요 없는 명백한 오류는 즉시 고치고, 스레드에는 👀 리액션만 남긴 채 전부 열어 둔다. 확인이 필요한 건은 채팅으로 보고. GitHub 코멘트와 스레드 resolve 는 호출자가 지시할 때만 한다. 머지 가능해지면 알리고 머지는 하지 않는다. 새로 리뷰하는 게 아니라 받은 리뷰에 대응하는 쪽(내 diff 를 검토받는 건 /rocky:review). 재리뷰를 기다리지 않는다.
...
1. **이 커맨드 호출 자체가 수정·커밋·푸시 승인이다.** 채팅에 사후 요약만 올린다. 매
   수정마다 확인을 받지 않는다. **코멘트와 resolve 는 여기에 포함되지 않는다** (원칙 4).
...
4. **스레드를 건드리지 않는다 — 코멘트도, resolve 도.** GitHub 에 남는 글은 되돌려도 알림이
   ...
   코멘트에 👀 리액션**을 달아 "봤다" 만 표시하고(5단계), 처리 내역은 채팅으로 보고한다.
   - **에이전트는 사용자 계정으로 쓴다.** `gh` 가 오너 토큰을 쓰므로 코멘트도 resolve 도 전부
     오너 이름으로 남는다 — 밖에서 보면 누가 한 것인지 구분되지 않는다. 그래서 둘을 나누는
     방법은 **어떤 행위를 하지 않기로 정해 두는 것**뿐이다.
   - **resolve 는 사용자 행위다.** 스레드를 닫는 것은 "내가 검토했다" 는 기록이고, 에이전트가
     닫아 버린 스레드는 사용자가 다시 열어 보지 않는다. 에이전트가 닫기 시작하면 그 기록은
     그냥 사라진다. 그래서 닫는 행위를 사용자에게 돌려준다 — 채팅에서 "resolve 해" 라고
     지시할 때만 에이전트가 닫는다.
   - 코멘트도 같다. **호출자가 승인한 경우에만** 만든다 (7단계). 그 밖에는 한 줄도 남기지 않는다.
5. **모든 판정의 축은 "호출자 판단이 필요한가" 다.**
   - 필요 없다 → 즉시 고친다. 물어보지 않는다.
   - 필요하다 → 손대지 않고 채팅으로 보고한다. 어느 쪽이든 스레드는 열린 채 남는다.
...
| 판정 | 조건 | 처리 |
| --- | --- | --- |
| **즉시 수정** | 호출자 판단이 필요 없는 명백한 오류 | 고치고 👀 만 (코멘트·resolve 없음) |
| **무효** | 이미 해결됐거나 대상 코드가 사라짐 | 👀 만 (코멘트·resolve 없음) |
| **확인 필요** | 호출자가 확인해야 하는 사항 | 손대지 않고 👀 + 채팅으로 보고 |
...
**애매하면 [확인 필요]로 보낸다.** 판단이 필요한 것을 임의로 고쳐 버리면 호출자는 그 결정이
있었다는 사실조차 모른다 — 반대 방향의 실수(물어봐서 한 번 더 확인받는 것)는 비용이 훨씬 싸다.
...
판정과 무관하게, 처리한 모든 스레드의 **첫 코멘트**에 👀 리액션을 단다. 이것이 이 커맨드가
GitHub 에 남기는 유일한 흔적이다 — 코멘트도, resolve 도 하지 않는다.

```bash
# COMMENT_ID = 1단계 수집 결과의 nodes[].comments.nodes[0].id (스레드 id 가 아니다)
for COMMENT_ID in $SEEN_IDS; do
  gh api graphql -f id="$COMMENT_ID" -f query='
  mutation($id:ID!){ addReaction(input:{subjectId:$id, content:EYES}){ reaction{ content } } }
  '
done
```

- 같은 코멘트에 이미 👀 가 달려 있어도 무해하다 — 중복 호출을 신경 쓰지 않는다.
...
수정 3 / 확인 필요 1 / 무효 1
  ...
  게이트 통과 · 커밋 abc1234 푸시 · 스레드 5건 👀 · 전부 열림 — resolve 는 확인 후
```
...
- **지적이 틀렸다(반론 승인)** → 한국어 근거 코멘트 1개를 그 스레드에 남긴다.
  **이것이 이 커맨드가 코멘트를 만드는 유일한 경우다.** 코멘트를 남겨도 스레드는 열어 둔다
  (사용자가 "resolve 해" 라고 하면 그때 닫는다).
...
- **고쳐야 한다 / 어느 방향으로 고칠지 정해졌다** → [즉시 수정] 으로 전환하고 3단계로 돌아가
  처리한다. 고친 뒤에도 스레드는 열어 둔다.
- **맞는 지적이지만 이 PR 범위 밖이다** → 후속 작업으로 남긴다(보드 항목 / 이슈). 어디로
  옮겼는지 코멘트를 남길지는 사용자가 정한다.
- **사용자가 답하지 않고 넘어갔다** → 그대로 두고 끝낸다. 8단계 머지 판정은 열린 스레드 수와
  무관하게 진행한다.
...
- 조건: checks 전부 통과 · `mergeStateStatus` 가 `CLEAN` 또는 `UNSTABLE`. **열린 스레드 수는
  판정 조건이 아니다** — 스레드를 닫는 것은 사용자 몫이므로(원칙 4), 열린 건수는 보고에만
  적는다("스레드 N건 열림 — resolve 하시면 머지 가능").
