...
에이전트 코딩 세션을 위한 **개인용 MCP toolkit** — OpenAPI / Swagger 명세 캐시-우선 탐색, SEO 메타 검증, Notion 페이지 캐시, 에이전트 워크로그(기록↔정리), 그리고 에이전트와 호출자가 웹브라우저로 함께 보는 공유 todo 보드(`rocky-todo` 데몬)를 하나의 Bun 패키지로 묶었다. 이름은 *Project Hail Mary* 의 Rocky (스펙을 번역해 주는 엔지니어) 에서.
...
여기에 더해 **`rocky-todo` 상주 데몬** (`bin/rocky-todo` → `src/todo/`, v0.13) 이 별개 프로세스로 공유 todo / 스크래치패드 보드를 서빙한다 — 에이전트는 `/mcp` (streamable HTTP, 도구 5개) 또는 CLI 로, 호출자는 React 웹 UI (`http://127.0.0.1:8636`, SSE 실시간) 로 같은 보드를 본다. 계층/섹션/보드 + 우선순위/라벨/마감/링크 + 처리중(actor) 표시 + 전 변경 히스토리, 삭제 없음(아카이브만). [설정 가이드](./docs/rocky-todo.md).
...
rocky-todo 데몬은 위 표와 별개의 프로세스로 `todo_list` / `todo_write` / `todo_status` / `note_list` / `note_write` 5 도구를 자신의 `/mcp` 엔드포인트에 노출한다 (호스트별 등록: [`docs/rocky-todo.md`](./docs/rocky-todo.md)).
...
- **훅** (`hooks/hooks.json`) — `SessionStart`: 활성 소울(페르소나) 자동 주입 + 설치된 statusline 스크립트 자동 동기화. `UserPromptSubmit`: rocky-todo 보드에서 호출자(사람)가 바꾼 내용을 다음 발화 때 자동 주입 (`todo.watch` 로 토글). `Stop`: 매 턴 종료 시 `kind:"turn"` 워크로그 자동 기록 (결정론적, LLM 미사용; `worklog.autoCapture` 로 토글).
...
- **스킬** (`skills/`) — `writing-cc-plugin`: Claude Code 플러그인 작성 가이드 + 매니페스트·컴포넌트·배포 레퍼런스. `delegating-to-codex`: 자기완결 task 를 headless OpenAI 모델 (`codex` CLI) 에 위임하는 패턴 + 가드레일 (`/codex` 커맨드가 얹히는 메커니즘 레이어). `todoist`: 세션에 연결된 Todoist MCP 로 현재 레포의 작업 목록을 파악·등록·마감하는 연동 스킬 — 다음 작업 제안은 Todoist + git + worklog 교차, 쓰기는 컨벤션 + 확인 게이트. `todo`: rocky-todo 공유 보드 사용 가이드 — 도구 게이트(MCP 우선, CLI 폴백) + 에티켓(작업 착수 시 start → 웹 UI 처리중 뱃지, 완료 시 done, 링크 첨부) + 가드레일(삭제 없음).
...
| [`docs/rocky-todo.md`](./docs/rocky-todo.md) | 사람 | rocky-todo 데몬 — 호스트별 MCP 등록 / daemon install / CLI 표면 |
