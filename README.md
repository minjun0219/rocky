...
- **스킬** (`skills/`) — `writing-cc-plugin`: Claude Code 플러그인 작성 가이드 + 매니페스트·컴포넌트·배포 레퍼런스. `delegating-to-codex`: 자기완결 task 를 headless OpenAI 모델 (`codex` CLI) 에 위임하는 패턴 + 가드레일 (`/codex` 커맨드가 얹히는 메커니즘 레이어). `todoist`: 세션에 연결된 Todoist MCP 로 현재 레포의 작업 목록을 파악·등록·마감하는 연동 스킬 — 다음 작업 제안은 Todoist + git + worklog 교차, 쓰기는 컨벤션 + 확인 게이트. (공유 보드 사용 가이드 `board` 스킬은 동반 플러그인 rocky-todo 로 이동.)
