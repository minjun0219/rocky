...
- **MVP scope violation**: a change that pulls in anything listed under *Out* in `AGENTS.md` "Scope" — YAML OpenAPI stream parsing, multi-spec merge, mock servers, full SDK codegen, UI, or re-adding an archived domain (journal / mysql / notion / spec-pact / pr-watch / opencode plugin) without an explicit request.
