...
- **`journal` → `worklog` 개명 (v0.9, 2026-07)**: 도구 4 종은 `worklog_*` 로, env / config 는 `ROCKY_WORKLOG_DIR` / `worklog.dir` 로 rename (개수·역할 불변). 정리 레이어는 외부 wiki (`/curate` + `wikiDir`) 를 버리고 **워크로그 자체에 `kind:"digest"` 앵커 항목을 남기는 `/recall`** 로 교체 — `wikiDir` / `ROCKY_JOURNAL_WIKI_DIR` 제거. 동시에 Claude Code plugin 의 `Stop` hook 이 매 턴 `kind:"turn"` 항목을 결정론적으로 자동 기록한다 (`worklog.autoCapture`, 기본 on) — 아래 "백그라운드 저널 캡처" 후보의 유보를 hook 방식으로 해소한 셈.
