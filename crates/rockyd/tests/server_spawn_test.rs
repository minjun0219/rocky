...
use rockyd::spawnctl::{SpawnFailedError, SpawnFn, SpawnInput};
use serde_json::json;
...
fn use_handle(
    ...
    real_path: Option<rockyd::server::RealPath>,
) -> Arc<ServerState> {
    rebuild(f, |o| {
...
async fn normalized_path_used_for_worktree_and_cwd() {
    ...
    let real_path: rockyd::server::RealPath = Arc::new(|p| {
        Ok(if p == "/link/repo" {
            "/real/repo".into()
        } else {
            p.to_string()
...
async fn realpath_failure_is_400() {
    let f = fx();
    let real_path: rockyd::server::RealPath =
