...
use rocky_core::types::{Todo, TodoLink};
// slug 판별·remote 파싱은 순수 문자열 처리라 코어에 있다 — CLI 도 같은 함수를 쓴다.
pub use rocky_core::github::{is_repo_slug, parse_repo_from_remote};
...
pub fn issue_body(description: &str, todo_ref: &str) -> String {
    let backlink = format!("— rocky `{todo_ref}`");
...
fn no_repo_message(board_key: &str) -> String {
    format!("board has no GitHub repo: {board_key} — 먼저 설정한다 (rocky board repo OWNER/NAME)")
}
...
pub async fn create_issue_for_todo(
    ...
    let board_ref = rocky_core::refs::ref_of(store, Some(&todo.board_id), todo.number, &todo.id)?;
