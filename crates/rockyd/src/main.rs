//! rockyd 단독 실행 — 플러그인 훅/CLI 가 띄우는 상주 데몬이자 개발용 진입점.
...
///
/// 실행 파일 옆을 보는 이유: 릴리스 tarball 이 `rocky`/`rockyd`/`dist/` 를 한
...
#[tokio::main]
async fn main() {
    if let Err(error) = rockyd::daemon::start_daemon(resolve_ui_dist()).await {
        eprintln!("rockyd: {error}");
        std::process::exit(1);
    }
}
