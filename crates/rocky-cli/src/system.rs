...
pub fn mcp_setup_guide(base_url: &str) -> String {
    format!(
        r#"rocky 데몬의 MCP 엔드포인트: {base_url}/mcp (streamable HTTP)

Claude Code:
  rocky 플러그인이 http 로 자동 등록한다 (plugin.json 의 mcpServers.rocky → {base_url}/mcp).
  데몬이 안 떠 있으면 도구가 안 붙는다 — rocky daemon start 로 켠 뒤 /mcp 패널에서 retry.
  과거 수동 http 등록이 있으면 제거: claude mcp remove rocky

opencode (~/.config/opencode/opencode.json):
  {{ "mcp": {{ "rocky": {{ "type": "remote", "url": "{base_url}/mcp" }} }} }}
