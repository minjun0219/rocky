...
///
/// 응답이 없거나 비정상이거나 rocky 데몬이 아니면 `None`.
...
pub fn ensure_daemon(ctx: &CliContext) -> Result<(), String> {
    ...
    std::process::Command::new(daemon_binary())
        ...
        .spawn()
        .map_err(|error| format!("rockyd 를 띄우지 못했다: {error}"))?;
    ...
    Err(format!(
        "rocky daemon did not start on port {} — check `rocky daemon status`",
        ctx.port
    ))
}
