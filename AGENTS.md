...
**rocky** (named after Project Hail Mary's Rocky) — a **single Bun package** with one full-surface stdio MCP server (`src/index.ts`) consumed by multiple hosts, plus a smaller host-agnostic **`openapi-mcp` standalone stdio CLI** (`bin/openapi-mcp` → `src/standalone.ts`, npm). The full-surface server is launched by the **Claude Code plugin** (marketplace; MCP server declared in `.claude-plugin/plugin.json`'s `mcpServers`) and can also be launched directly by **OpenAI Codex CLI** via `~/.codex/config.toml` (`bun run <repo>/src/index.ts`) and **opencode** via the `mcp` section of `opencode.json` (`type:"local"`, `command:["bun","run","<repo>/src/index.ts"]`). `src/core/` holds the shared OpenAPI core. The standalone CLI exposes the **same 7 openapi tools** (`openapi_get` / `openapi_refresh` / `openapi_status` / `openapi_search` / `openapi_envs` / `openapi_endpoint` / `openapi_tags`); the full-surface server additionally exposes `seo_validate` (OG / Twitter Card / JSON-LD / favicon meta validation via `ogpeek`, `src/core/seo-validate.ts`). From v0.5 the full-surface server also adds **four CLI-gated `notion_*` tools** (`notion_get` / `notion_refresh` / `notion_status` / `notion_extract`, `src/core/notion-handlers.ts`) — registered only when the official Notion CLI (`ntn`) is detected at startup; rocky never touches Notion tokens / OAuth itself, all page access is delegated to `ntn pages get` (`src/core/notion-cli.ts`), same policy as the `gh`-based slash commands. From v0.6 the full-surface server also adds **four `worklog_*` tools** (`worklog_append` / `worklog_read` / `worklog_search` / `worklog_status`, `src/core/worklog.ts` + `src/core/worklog-handlers.ts`; renamed from `journal_*` in v0.9) — the **record (기록) layer**: an append-only local JSONL agent worklog. Unlike notion these are **not CLI-gated** (pure filesystem, no external dep) so they are always registered. From v0.9 the **Claude Code plugin's** `Stop` hook (`hooks/hooks.json` → `src/hooks/log-turn.ts`, transcript parsing in `src/hooks/transcript.ts`) deterministically (no LLM) appends a `kind:"turn"` worklog entry after every turn — toggle via `worklog.autoCapture` / env `ROCKY_WORKLOG_AUTO_CAPTURE` (default on), truncated to `worklog.captureMaxChars` (default 800). This turn auto-capture (and its `ROCKY_WORKLOG_AUTO_CAPTURE` toggle) is **Claude Code-only** — rocky ships no Codex/opencode hooks (both hosts *do* support hooks natively; rocky just doesn't wire them there yet) — even though the `worklog_*` tools themselves work on all three full-surface hosts. rocky only records / stores; the paired **organize (정리) layer** is the `/rocky:recall` slash command (host LLM, replacing the earlier `/curate`), which reads the worklog and incrementally distills an anchor history digest — each digest is appended back into the worklog itself as a `kind:"digest"` entry (linking back to source entry ids) rather than written to an external wiki, so there is no more `wikiDir` and it stays distinct from Claude Code's native memory. Separately, the Claude Code plugin ships **slash commands** in `commands/` (`/rocky:finish` — `gh` CLI based; `/rocky:review-pr` — `gh` CLI based, drives the PR review loop (collect `reviewThreads` → classify fix/rebut/moot → fix + gates → one commit per round → `resolveReviewThread` → poll for re-review) until zero unresolved threads, holding rebuttals in a queue for a final confirm-then-comment pass and pushing a "mergeable" notification at the end (never merges); `/rocky:recall` — reads `worklog_*` and appends a `kind:"digest"` entry; `/rocky:codex` — delegates one task to Codex (`codex exec`) in an isolated worktree and supervises gates / MCP-surface / diff-scope before merging (no auto-merge); self-contained since v0.16 — the delegation mechanics that used to live in a `delegating-to-codex` skill are inlined here, since the official `openai/codex-plugin-cc` plugin now covers general Codex delegation and this command's remaining value is the worktree isolation + plugin-surface integrity check; `/rocky:opencode` — delegates one task to opencode (`opencode run`) in an isolated worktree and supervises gates / MCP-surface / diff-scope before merging (no auto-merge); from v0.17 the dispatch goes through a **companion runtime** (`src/opencode-companion.ts`, core in `src/core/opencode-{jobs,cli,runner,render}.ts`) that passes the prompt via `--prompt-file`, parses the `--format json` NDJSON stream, and supports `--background` by re-executing itself as a detached `job-worker` (the job file is the only parent↔worker IPC channel); its paired `/rocky:opencode-jobs` command does `status` / `result` / `cancel` over those jobs, scoped to the current Claude session via a `ROCKY_SESSION_ID` injected by a matcher-less `SessionStart` hook (`src/hooks/session-jobs.ts`, whose `SessionEnd` half reaps orphaned workers while keeping the records). Deliberately **no long-lived broker** — unlike `codex app-server`, `opencode run` is one-shot, so the entire broker / endpoint / PID layer that the official Codex plugin needs is absent here; `/rocky:issue` — `gh` CLI based, captures a rocky feature idea / bug raised while working in *another* repo as a GitHub Issue on `minjun0219/rocky` (gathers session context, checks for duplicate open issues, confirms a draft before creating; never auto-creates); `/rocky:soul` — lists / sets / previews / scaffolds souls (personas), writing the `soul` key in `rocky.json`; `/rocky:statusline` — installs / checks / removes the bundled statusline (see below); none are MCP tools) and two **skills** in `skills/` — `writing-cc-plugin` (a Claude Code plugin authoring guide + full manifest/component reference, `SKILL.md` + `reference.md`, distilled from the official `/ko/plugins` + `/ko/plugins-reference` docs) and `todoist` (works the user's Todoist as the current repo's task ledger via whatever Todoist MCP tools are connected to the session — next-task proposals cross-referencing Todoist + git state + the rocky worklog, convention-gated task registration and confirmed completion; rocky ships no Todoist access itself — with no Todoist tools connected the skill stops and says so). These slash commands and the skills are Claude Code-only; rocky exposes only its MCP tools to Codex and opencode **today**. This is a rocky wiring choice, not a host limitation — as of 2026 both Codex and opencode natively support commands / hooks / skills / subagents (and Codex also `.codex-plugin/plugin.json` bundles + a marketplace), so these surfaces are portable, just not yet shipped there. See FEATURES.md's *호스트 지원 매트릭스* for the full mechanism-by-host + coverability breakdown. PR review handling is `/rocky:review-pr`; Claude Code's built-in `/autofix-pr` (cloud session + GitHub App webhooks) remains a separate option for CI-failure autofixes — the former `/pr-watch` command was removed in v0.8. Also Claude Code-only: **souls** (personas) — markdown files (frontmatter `name`/`description` + persona body) at bundled presets `souls/<name>.md` (`rocky` / `senior` / `terse`) or custom `~/.config/rocky/souls/<name>.md` (same name → custom wins, identity = filename stem); `rocky.json`'s optional `soul` field (`src/core/soul.ts`, project overrides user) names the active one, and a new `SessionStart` hook (`hooks/hooks.json` → `src/hooks/inject-soul.ts`, `matcher: "startup|clear|compact"` so it (re)injects on a fresh/cleared/compacted context but skips `resume`) resolves and injects it as `additionalContext` — fail-open, and a layer over AGENTS.md/CLAUDE.md's gates/safety, never an override; default (no `soul` set) is vanilla, no injection. The optional `callsign` field (single line, non-blank, at most 40 chars, Korean/spaces OK, project overrides user) names what the soul calls the user — the hook appends it to the injected soul context as one directive line that beats the soul body's default form of address; ignored when no `soul` is set. `/rocky:soul` switches the active soul (asking for a callsign during setup) and `/rocky:soul call` sets/clears the callsign alone. Also Claude Code-only: a bundled **statusline** — three `statusLine` script templates (`statusline/<name>.sh`, POSIX sh + jq: `duo` 2-line default (cwd+branch / model+ctx+session-limit-remaining+reset timer), `mini` 1-line compact, `full` 3-line adding git dirty/ahead-behind, ctx/left threshold warning colors, session cost (+hourly rate)/line churn/elapsed (template breakdown in `docs/statusline.md`)). Claude Code's `statusLine` setting only lives in user `settings.json` (plugin `settings.json` supports only `agent`/`subagentStatusLine`), so `/rocky:statusline` installs a chosen template once — copying it to the stable path `~/.config/rocky/statusline.sh` and pointing `statusLine.command` there (never at the per-version plugin cache path) after a confirmed draft + timestamped settings backup — and a `SessionStart` hook (`matcher: "startup"` → `src/hooks/sync-statusline.ts`, core `src/core/statusline.ts`) reads the installed copy's `# rocky-statusline-template: <name>` header marker (fallback `duo`) and propagates plugin updates from that same template (no-op until installed; fail-open). Neither souls nor the statusline add MCP tools; `src/index.ts`'s surface and Codex and opencode are unaffected. The **rocky-todo** shared board daemon — bundled here in v0.13 — now lives in its **own repo/plugin** `minjun0219/rocky-todo` (extracted 2026-07-25), installed via the same rocky marketplace (2nd entry, github source, `dependencies:["rocky"]`) but with an independent lifecycle and its own daemon/web-UI/CLI/hooks. rocky itself no longer ships any todo code, daemon, web UI, or the `notify-todo` hook, and dropped its react/react-dom/zustand deps. `rocky.json` still **tolerates** a `todo` block (rocky ignores it — the rocky-todo daemon consumes it) because the file is shared across the ecosystem. No workspaces, no `packages/` — one `package.json`, one `tsconfig.json`.
Previous toolkit surfaces (journal / mysql / spec-pact / pr-watch + rocky / grace / mindy agents + 5 skills) live on [`archive/pre-openapi-only-slim`](https://github.com/minjun0219/rocky/tree/archive/pre-openapi-only-slim); the former native opencode plugin is archived in-tree at [`.archive/agent-toolkit-opencode/`](./.archive/agent-toolkit-opencode) (excluded from all gates). That archive was an in-process `@opencode-ai/plugin` surface exposing only the old openapi tools; current opencode support is not a revival of that plugin. opencode now consumes rocky exactly like other hosts: by spawning the host-agnostic stdio MCP server (`src/index.ts`) from `opencode.json`, which exposes the full surface. Domains re-enter in follow-up PRs via one of two shapes (plugin-bound handlers, or a separate CLI entry alongside `openapi-mcp`). The shape is decided per domain at re-introduction time. **notion was the first re-added domain (v0.5, plugin-bound + `ntn` CLI-gated)** — its shape (external CLI delegation, no tokens in rocky) is the template for future auth-bearing domains. **journal was the second (v0.6, plugin-bound, always-on)** — split into a record layer (`journal_*` MCP tools, deterministic storage) and an organize layer (`/curate` slash command, host-LLM distillation into a configured wiki); this record↔organize split is the template for memory-shaped domains that must stay distinct from Claude Code's native memory. **journal was renamed `worklog` in v0.9**, which also added a third piece — a deterministic `Stop` hook that auto-captures every turn (`kind:"turn"`) — while the organize layer moved from an external wiki (`/curate`) to an in-worklog anchor digest (`/rocky:recall`, `kind:"digest"`); the record↔organize split itself is unchanged, only the organize target moved from an external file tree into the worklog's own append log.
...
├── docs/statusline.md                      번들 statusline 템플릿 3종(duo/mini/full)의 표시 내용·임계값 문서
├── commands/                               ★ Claude Code plugin 슬래시 커맨드 (자동 발견, gh CLI 기반, MCP tool surface 와 별개)
│   ├── finish.md                           `/rocky:finish` — 게이트 → 커밋 → 푸시 → PR 생성 (이후 리뷰 대응은 /rocky:review-pr)
...
│   ├── recall.md                           `/rocky:recall` — worklog_* 를 읽어 앵커 히스토리 다이제스트(kind:"digest")로 증분 요약 (정리 레이어, gh 불필요, v0.9 에서 curate.md 대체)
│   ├── codex.md                            `/rocky:codex` — task 를 Codex(codex exec)에 위임(격리 worktree) + Claude 감시(게이트/표면/스코프), 자동 병합 X
│   ├── opencode.md                         `/rocky:opencode` — task 를 opencode(opencode run)에 위임(격리 worktree) + Claude 감시(게이트/표면/스코프), 자동 병합 X. dispatch 는 src/opencode-companion.ts 경유 (--prompt-file, --background 지원)
│   ├── opencode-jobs.md                    `/rocky:opencode-jobs` — 백그라운드 위임 잡 status/result/cancel (companion 패스스루, 잡 생성 X)
│   ├── issue.md                            `/rocky:issue` — 다른 레포에서 떠오른 rocky 개선 아이디어/버그를 minjun0219/rocky GitHub Issue 로 캡처 (gh CLI 기반, 맥락 수집 + 유사 이슈 조회 + 초안 확인 후 생성)
...
│   └── hooks.json                          `SessionStart` (matcher `startup|clear|compact`) → `bun run ${CLAUDE_PLUGIN_ROOT}/src/hooks/inject-soul.ts` (소울 자동 주입) + `SessionStart` (matcher `startup`) → `bun run ${CLAUDE_PLUGIN_ROOT}/src/hooks/sync-statusline.ts` (statusline 자동 동기화) + `SessionStart`/`SessionEnd` (**matcher 없음** — resume 포함 전체) → `bun run ${CLAUDE_PLUGIN_ROOT}/src/hooks/session-jobs.ts` (v0.17, 위임 잡 세션 배선/고아 정리) + `Stop` → `bun run ${CLAUDE_PLUGIN_ROOT}/src/hooks/log-turn.ts` (v0.9, 턴 자동 기록)
...
│   └── todoist/                            `/rocky:todoist` — 세션에 연결된 Todoist MCP 로 현재 레포 작업 목록 파악·등록·마감
│       └── SKILL.md                        도구 게이트(없으면 중단·안내) + 레포↔프로젝트 휴리스틱 매핑 + 3 워크플로(next: Todoist+git+worklog 교차 / 등록: 컨벤션+차등 확인 게이트 / 완료: 확인 후 complete) + 가드레일
├── souls/                                  ★ 번들 프리셋 소울 (rocky / senior / terse), 커스텀은 ~/.config/rocky/souls/
│   ├── rocky.md                            frontmatter(name/description) + 페르소나 본문
│   ├── senior.md                           〃
│   └── terse.md                            〃
├── statusline/                             ★ 번들 statusline 템플릿 — /rocky:statusline 이 골라 ~/.config/rocky/statusline.sh 로 설치 (헤더 마커로 훅 sync)
│   ├── duo.sh                              2줄 (cwd+branch / model+ctx+left+reset) — 기본, POSIX sh + jq
│   ├── mini.sh                             1줄 컴팩트 (cwd+branch+model+ctx+left)
│   └── full.sh                             3줄 (duo + 세션 비용/변경 라인 수/경과)
├── bin/
...
└── src/
    ...
    ├── opencode-companion.ts               ★ `/rocky:opencode` 위임 런타임 CLI (v0.17) — check / task / job-worker / status / result / cancel. MCP tool 아님 (슬래시 커맨드가 Bash 로 호출). `task --background` 는 자기 자신을 `job-worker` 로 detached 재실행
    ...
    │   ├── sync-statusline.ts              SessionStart hook entry — 번들 statusline 스크립트를 설치 안정 경로로 동기화 (미설치면 no-op, fail-open)
    │   ├── log-turn.ts                     Stop hook entry — shouldCapture(env/config) → extractTurn → worklog.append({ kind: "turn" }) (v0.9)
    │   ├── session-jobs.ts                 SessionStart/SessionEnd hook entry (v0.17) — CLAUDE_ENV_FILE 에 ROCKY_SESSION_ID export / 세션 종료 시 진행 중 위임 잡 프로세스 그룹 정리 (기록은 보존, fail-open)
    ...
    └── core/                               ← 구 @minjun0219/openapi-core
        ...
        ├── statusline.ts                   statusline 코어 — 번들 스크립트 → 안정 경로 sync (순수/DI, hook + /rocky:statusline 이 소비)
        ...
        ├── opencode-jobs.ts                위임 잡 파일 저장소 (v0.17) — state.json 인덱스 + jobs/<id>.json payload + .log, seq 정렬키, prefix 매칭, maxJobs prune (부모↔detached 워커의 유일한 IPC 채널)
        ├── opencode-cli.ts                 opencode CLI 어댑터 (v0.17) — buildRunArgs / detectOpencode / NDJSON 요약 / killProcessGroup / `node:child_process` spawn executor (fakeable). Bun.spawn 이 아닌 이유는 `detached` 가 필요해서 — opencode 를 자체 프로세스 그룹 리더로 띄워야 타임아웃 시 손자까지 끊는다
        ├── opencode-runner.ts              위임 실행 루프 (v0.17) — runJob (foreground·워커 공용, 예외를 잡 레코드로 흡수) + killJobProcess (kill(-pid) 프로세스 그룹)
        ├── opencode-render.ts              status / result 사람용 렌더링 (v0.17, 순수 함수 — now 주입)
...
**In**: OpenAPI / Swagger spec 캐시 + endpoint search + tag list + cross-spec scoped search + `host:env:spec` registry (`rocky.json`, project > user precedence), 공유 7 openapi tool 을 full-surface server (`src/index.ts`, consumed by Claude Code plugin + Codex CLI + opencode) 와 standalone CLI (`src/standalone.ts`) 에 공유, 단일 Bun 패키지. 모든 openapi handler 는 `src/core/handlers.ts` 한 곳에 정의 — 두 진입점 간 drift 방지. 추가로 **`seo_validate`** (단일 URL 의 OG / Twitter Card / JSON-LD / favicon 메타를 `ogpeek` 으로 검증, 기본 SSRF 가드 — IP literal 기준 private/loopback 차단, `rocky.json` 의 `seo.allowPrivateHosts` / `seo.timeoutMs` 기본값, 도구 인자 우선) 는 full-surface server 에만 등록 (handler `src/core/seo-validate.ts`, standalone CLI 미포함). 그리고 **`notion_*` 4 도구** (`notion_get` / `notion_refresh` / `notion_status` / `notion_extract`) 는 full-surface server 에만, 그리고 **공식 Notion CLI (`ntn`) 가 기동 시 탐지될 때만** 등록 — 페이지 접근은 전부 `ntn pages get <id> --json` 위임 (rocky 는 토큰 / OAuth 를 직접 다루지 않음), 캐시는 `ROCKY_NOTION_CACHE_DIR` 아래 page 당 `.json` + `.md` 두 파일, `notion_refresh` 는 기존 캐시 대비 heading-section diff (자체 LCS, 외부 dep 0) 를 함께 반환. `buildServer({ notionCli })` 로 executor 를 주입해 테스트에서 fake 로 대체. 그리고 **`worklog_*` 4 도구** (`worklog_append` / `worklog_read` / `worklog_search` / `worklog_status`, v0.9 에서 `journal_*` 개명) 는 full-surface server 에만, **CLI-gate 없이 항상** 등록 — append-only 로컬 JSONL 기록 레이어 (`src/core/worklog.ts`, 외부 dep 0, `resolveCacheKey` 재사용). 저장은 프로젝트별 (`~/.config/rocky/worklog/<project-key>/worklog.jsonl`, `ROCKY_WORKLOG_DIR` / `rocky.json` 의 `worklog.dir` 로 변경). v0.9 부터 `Stop` hook (`hooks/hooks.json` → `src/hooks/log-turn.ts`) 이 매 턴 종료 시 결정론적으로(LLM 미사용) `kind:"turn"` 항목을 자동 append 한다 — 토글은 `worklog.autoCapture` / env `ROCKY_WORKLOG_AUTO_CAPTURE` (기본 on), truncate 길이는 `worklog.captureMaxChars` (기본 800). 짝이 되는 정리(整理) 레이어는 rocky 도구가 아니라 **`/rocky:recall` 슬래시 커맨드** (v0.9 에서 구 `/curate` 대체) — 워크로그를 읽어 마지막 digest watermark(`lastDigestAt`) 이후 항목만 증분 요약하고, 새 항목 수(`worklog.digestThreshold`, 기본 40) 에 따라 Haiku/Sonnet 서브에이전트를 골라 앵커 히스토리 다이제스트를 만들어 `kind:"digest"` 한 줄로 append 한다 — 더 이상 외부 wiki 는 없다, 다이제스트는 워크로그 자체 안에 산다. rocky 는 기록·저장만, 증류(LLM)는 호스트 몫이라 네이티브 메모리와 역할이 겹치지 않는다. `buildServer({ worklog })` 로 tmpdir 워크로그 주입 가능. 그리고 **번들 statusline** (Claude Code 전용, MCP tool 아님) — 스크립트 원본은 템플릿 `statusline/<name>.sh` 3종 (`duo`/`mini`/`full`), 설치는 `/rocky:statusline` 이 고른 템플릿을 안정 경로 `~/.config/rocky/statusline.sh` 복사 + user `settings.json` 의 `statusLine` 1회 지정 (초안 확인 + 타임스탬프 백업, 캐시 경로 직접 지정 금지), 업데이트 전파는 `SessionStart` hook (`src/hooks/sync-statusline.ts` → `src/core/statusline.ts`) 이 설치본 헤더의 템플릿 마커로 같은 템플릿에서 수행 (미설치면 no-op). 그리고 **opencode 위임 런타임** (v0.17, Claude Code 전용, **MCP tool 아님**) — `/rocky:opencode` 의 dispatch 를 맡는 companion CLI (`src/opencode-companion.ts`) + 코어 4종 (`src/core/opencode-jobs.ts` 잡 파일 저장소 / `opencode-cli.ts` argv·NDJSON 어댑터 / `opencode-runner.ts` 실행 루프·프로세스 그룹 kill / `opencode-render.ts` 순수 렌더러) + 세션 배선 훅 (`src/hooks/session-jobs.ts`, SessionStart/SessionEnd, **matcher 없음** — `resume` 포함) + 조회 커맨드 `/rocky:opencode-jobs`. 프롬프트는 `--prompt-file` 로만 넘기고(셸 인용 회피), 출력은 `--format json` NDJSON 을 관대하게 파싱해 최종 텍스트 + opencode 세션 id 를 뽑는다. `--background` 는 자기 자신을 `job-worker` 로 `detached:true, stdio:"ignore"` 재실행하며, **잡 파일이 부모↔워커의 유일한 IPC 채널**이다 (그래서 `cancel` / `SessionEnd` 는 **워커 그룹과 opencode 그룹 둘 다** `kill(-pid)` 로 끊는다 — opencode 는 타임아웃 시 자기 손자(`bash` 등)까지 정리하려고 `detached` 로 자체 그룹의 리더가 되므로 워커 그룹만 끊으면 닿지 않는다. 그래서 잡에 워커 `pid` 와 opencode `childPid` 를 모두 기록한다). 인덱스의 read-modify-write 는 `state.lock`(mkdir 원자성)으로 **프로세스 간 직렬화**한다 — 없으면 부모의 잡 생성과 워커의 상태 갱신이 같은 스냅샷을 덮어써 인덱스에서 잡이 사라지고(실측: 동시 8건 중 6건 유실) 그 opencode 가 고아로 남는다. prune 은 **종료된 잡만** 지운다 (활성 잡을 지우면 워커가 완료를 기록하지 못한다). 상태는 `~/.config/rocky/jobs/<project-key>` (env `ROCKY_OPENCODE_JOBS_DIR` / `rocky.json` 의 `opencode.dir`), 외부 dep 0 (`node:fs`/`child_process`/`crypto`). opencode spawn 은 `Bun.spawn` 이 아니라 **`node:child_process`** 를 쓴다 — `detached` 지원이 필요하기 때문이며, 이 선택이 위의 프로세스 그룹 kill 전제를 성립시킨다. **의도적으로 브로커를 두지 않는다** — `codex app-server` 와 달리 `opencode run` 은 one-shot 이라 공식 Codex 플러그인의 broker/endpoint/PID 계층이 통째로 불필요하다. (**rocky-todo** 공유 보드 데몬은 v0.13 에 여기 번들됐다가 2026-07-25 별도 레포/플러그인 `minjun0219/rocky-todo` 로 분리 — rocky 본체 In 범위에서 제외. 같은 rocky 마켓의 2번째 entry(github source)로 서빙되고 `dependencies:["rocky"]` 로 rocky 를 동반한다.)

**Out**: mysql / spec-pact / pr-watch / agents / skills (전부 archive 브랜치 박제), the old native opencode plugin (`.archive/agent-toolkit-opencode/` 박제; distinct from current opencode stdio MCP host support), 나머지 도메인 재추가 (별도 PR), npm publish 자동화 (별도 PR). notion 은 재추가 완료 (v0.5), journal 은 재추가 완료 (v0.6, v0.9 에서 `worklog` 로 개명). **worklog 정리 결과를 rocky MCP 도구(`wiki_*`)로 노출 · worklog 를 standalone CLI 에 추가 · 네이티브 메모리로의 자동 승격/동기화 · 폴링 기반 자동 digest** 는 out — 기록은 `worklog_*` 도구 (+ `Stop` hook 자동 turn 캡처), 정리는 `/rocky:recall` 슬래시 커맨드(워크로그 내부 `kind:"digest"` entry)만. **Notion DB / child-page 재귀 · YAML frontmatter 파싱 · `ntn` 외 다른 Notion 접근 경로 (직접 API / MCP)** 는 out — 단일 페이지 캐시 + `ntn` 위임만. **rocky-todo 관련 일체** (데몬/웹UI/CLI/훅/도구)는 별도 레포 `minjun0219/rocky-todo` 소관 — rocky 본체 out. OpenAPI YAML stream parsing, full SDK code generation, multi-spec merge, mock servers / UI 도 모두 out.
