...
│   └── soul.md                             `/rocky:soul` — 소울(페르소나) 선택 (list/set/show/new), MCP tool 아님
├── hooks/                                  ★ Claude Code plugin 훅 (자동 발견, MCP tool surface 와 별개)
│   └── hooks.json                          `SessionStart` → `bun run ${CLAUDE_PLUGIN_ROOT}/src/hooks/inject-soul.ts` (소울 자동 주입) + `Stop` → `bun run ${CLAUDE_PLUGIN_ROOT}/src/hooks/log-turn.ts` (v0.9, 턴 자동 기록)
