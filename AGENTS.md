...
├── docs/rocky-todo.md                      rocky-todo 데몬 보조 문서 — 호스트별 MCP 등록 + daemon install + CLI 표면
...
│   └── todo/                               `/rocky:todo` — rocky-todo 공유 보드 사용 가이드 (MCP 우선 + CLI 폴백)
│       └── SKILL.md                        도구 게이트 + 보드 결정(레포 이름) + 에티켓(start→done, actor, 링크 첨부) + 가드레일(삭제 없음)
...
│   └── rocky-todo                          `#!/usr/bin/env bun` shebang → src/todo/cli 의 runCli
└── src/
    ...
    │   └── notify-todo.ts                  UserPromptSubmit hook entry — rocky-todo /api/changes 를 세션별 커서로 읽어 사람(비-에이전트) 변경만 additionalContext 주입 (fail-open, 데몬 미기동 시 no-op)
    ├── todo/                               ★ rocky-todo — 공유 todo/스크래치패드 데몬 (별도 프로세스, src/index.ts 표면과 무관)
    │   ├── daemon.ts                       Bun fullstack 진입점 — 단일 인스턴스 가드 + / (React UI) + /api/* + /mcp 서빙
    │   ├── server.ts                       buildTodoServer — REST 라우트 + SSE 허브 (DI, 테스트에서 fetch 직접 호출)
    │   ├── mcp.ts                          MCP 5 도구 + WebStandard streamable HTTP handler (stateless, 요청당 인스턴스)
    │   ├── store.ts                        SQLite 스토어 — CRUD + 계층/섹션 + 아카이브 + history 자동 기록 + change 이벤트
    │   ├── cli.ts                          CLI — 얇은 HTTP 클라이언트 + 데몬 온디맨드 spawn + 컴팩트 출력 (runCli)
    │   ├── actor.ts                        actor 감지(호스트 마커) + board key 유추(git remote > toplevel > cwd)
    │   ├── config.ts                       런타임 설정 해석 (env > user rocky.json todo > 기본 8636/~/.config/rocky/todo)
    │   ├── notify.ts                       UserPromptSubmit 훅 순수 로직 — 사람 변경 필터 + 컨텍스트 포맷 + 세션별 커서 저장(prune)
    │   ├── tailscale.ts                    tailscale serve 연동 — on/off/status + expose 에 tailscale-serve 채널 있을 때 데몬 기동 시 ensure
    │   ├── launchd.ts                      daemon install/uninstall/status (launchctl bootstrap, KeepAlive)
    │   ├── ui/                             React 웹 UI — index.html + main.tsx + zustand store + components/ (서빙 시 자동 번들)
    │   └── *.test.ts                       store / server / mcp / cli / actor / config 테스트
