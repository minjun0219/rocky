...
│   ├── finish.md                           `/rocky:finish` — 게이트 → 커밋 → 푸시 → PR 생성 (PR 감시는 빌트인 /autofix-pr 위임)
